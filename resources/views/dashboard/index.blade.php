...
<div class="main-content">
    ...
    <style>
        ...
        .trading-close-btn {
            ...
            height: 44px;
            background: #8bc905;
            border: 2px solid rgba(139, 201, 5, .3);
            border-radius: 50%;
            color: #0C3A30;
            cursor: pointer;
            display: flex;
            align-items: center;
            justify-content: center;
            box-shadow: 0 0 20px rgba(139, 201, 5, .4);
        ...
        .trading-card {
            ...
            border-radius: 16px;
            overflow: hidden;
            box-shadow: 0 25px 80px rgba(0, 0, 0, .7), 0 0 40px rgba(139, 201, 5, .15);
            animation: cardSlideUp .5s ease;
            border: 1px solid rgba(139, 201, 5, .2)
        }
        ...
        .trader-avatar {
            ...
            justify-content: center;
            font-size: 1.875rem;
            box-shadow: 0 4px 12px rgba(139, 201, 5, .25)
        }
        ...
        .trading-cta-btn {
            ...
            box-shadow: 0 6px 20px rgba(139, 201, 5, .4);
            transition: all .3s ease;
            margin-bottom: 1.5rem;
        ...
        .guide-toggle:hover {
            border-color: #8bc905;
            box-shadow: 0 2px 8px rgba(139, 201, 5, .15)
        }
        ...
        .guide-step:hover {
            border-color: #8bc905;
            box-shadow: 0 2px 8px rgba(139, 201, 5, .1)
        }
    ...
    <div class="flex items-center gap-4 bg-[#f0fdf4] px-2 py-3 rounded-lg shadow-sm w-full sm:w-auto">
        ...
        <div class="relative">
            ...
            @php
                ...
                | Supports:
                | - filename.png
                | - uploads/profile_pics/filename.png
                | - /uploads/profile_pics/filename.png
                | - full URL accidentally saved
                |--------------------------------------------------------------------------
                */
                ...
                if ($profilePic) {
                    ...
                    $cleanPath = str_replace('\\', '/', $profilePic);

                    // remove domain if saved accidentally
                    $cleanPath = preg_replace('/^https?:\/\/[^\/]+\//', '', $cleanPath);

                    // remove leading slash
                    $cleanPath = ltrim($cleanPath, '/');

                    // if only filename exists
                    if (!str_contains($cleanPath, 'uploads/profile_pics/')) {
                        $cleanPath = 'uploads/profile_pics/' . basename($cleanPath);
                    }

                    $profileUrl = asset($cleanPath) . '?v=' . time();
                }
            @endphp
        ...
        <style>
            ...
            .qa-btn-fund:hover {
                ...
                box-shadow: 0 4px 16px rgba(139, 201, 5, .4)
            }
        ...
        <style>
            ...
            .activity-exit {
                animation: fadeSlideOut .6s ease-in forwards
            }
        ...
        <script>
            ...
            let nameIndex = 0;
            const randomName = () => {
                const n = mixedNames[nameIndex % mixedNames.length];
                nameIndex++;
                return n;
            };
            const activities = [{
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <style>
                ...
                @keyframes pulseAnim {
                    0% {
                        transform: scale(1);
                        box-shadow: 0 0 0 0 rgba(238, 19, 37, .4)
                    }

                    70% {
                        transform: scale(1.2);
                        box-shadow: 0 0 0 6px rgba(238, 19, 37, 0)
                    }

                    100% {
                        transform: scale(1);
                        box-shadow: 0 0 0 0 rgba(238, 19, 37, 0)
                    }
                }
                        ...
                        fetch('/user/fluctuations-json?refresh=' + new Date().getTime(), {
                            ...
                            .then(r => r.json()).then(data => {
                                for (const id in data) {
                                    const s = document.getElementById('fluctuation-' + id);
                                    if (s) {
                                        s.innerHTML = data[id].html;
                                        s.className = data[id].class;
                                    }
                                }
                            });
                    };
                    ...
                    <div class="p-5 bg-white/80 backdrop-blur-sm rounded-2xl">
                        ...
                        <div class="flex gap-3">
                            <a href="{{ route('user.deposit') }}" class="qa-btn qa-btn-fund flex-1 justify-center">
                                <span class="qa-btn-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2">
