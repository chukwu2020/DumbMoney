...
<div class="col-span-2">
    ...
    <input type="file" name="server_profile_image"
        class="border rounded-lg p-2 text-sm w-full mt-1">
                ...
                <div>
                    <label class="text-xs font-semibold text-gray-600">Server Name</label>
                    <input type="text" name="server_name" class="border rounded-lg p-2 text-sm w-full mt-1" required>
                </div>

                <div>
                    <label class="text-xs font-semibold text-gray-600">Active Members</label>
                    <input type="number" name="active_members" class="border rounded-lg p-2 text-sm w-full mt-1" required>
                </div>

                <div>
                    <label class="text-xs font-semibold text-gray-600">Copying Trades</label>
                    <input type="number" name="copying_trades" class="border rounded-lg p-2 text-sm w-full mt-1" required>
                </div>

                <div>
                    <label class="text-xs font-semibold text-gray-600">Total Profit ($)</label>
                    <input type="number" step="0.01" name="profit_margin"
                        class="border rounded-lg p-2 text-sm w-full mt-1"
                        placeholder="e.g 1500000.00" required>
                </div>

                <div>
                    <label class="text-xs font-semibold text-gray-600">Win Rate (%)</label>
                    <input type="number" step="0.01" name="win_rate"
                        class="border rounded-lg p-2 text-sm w-full mt-1"
                        min="0" max="100" placeholder="e.g 87.50">
                </div>

                <div class="flex items-center gap-2 mt-6">
                    <input type="checkbox" name="copy_trading_enabled" value="1" checked>
                    <label class="text-xs font-semibold text-gray-600">Enable Copy Trading</label>
                </div>
    ...
    <td class="px-4 py-3 flex items-center gap-3">
        ...
        @endif
        <div>
            <div class="font-semibold">{{ $feed->server_name }}</div>
    ...
    <td class="px-4 py-3 text-center">
        <div class="flex justify-center gap-2">
            ...
            <a href="{{ route('admin.feeds.edit', $feed->id) }}"
               class="px-3 py-1 text-xs bg-blue-100 text-blue-600 rounded-lg">
               Edit
            </a>
