...
<style>
    ...
    .btn-brand {
        background:var(--brand-green); color:var(--brand-dark); font-weight:700;
    ...
    .data-table { width:max-content; min-width:100%; border-collapse:separate; border-spacing:0; font-size:.83rem; }
    .data-table thead th {
        background:var(--brand-dark) !important; color:#fff !important;
        padding:.75rem 1rem; text-align:left; font-weight:600; font-size:.7rem;
    ...
    <div style="background:#fff; border-radius:16px; border:1px solid #e5e7eb; box-shadow:0 1px 3px rgba(0,0,0,.06); overflow:hidden; min-width:0; max-width:100%;">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;padding:1rem 1.5rem;border-bottom:1px solid #f1f5f9;">
            ...
            <a href="{{ route('admin.payouts.create') }}"
               style="display:inline-flex;align-items:center;gap:4px;font-size:.75rem;font-weight:600;padding:.4rem 1rem;border-radius:8px;background:var(--brand-green);color:var(--brand-dark);text-decoration:none;"
               onmouseover="this.style.background='#8bc905'" onmouseout="this.style.background='var(--brand-green)'">
        ...
        <div class="tbl-shell">
            <table class="data-table">
                <thead>
                    <tr>
                        <th>ID</th><th>Date</th><th>Name</th><th>Amount</th>
                        <th>Processing</th><th>Plan</th><th>Account Type</th>
                        <th>Location</th><th>Status</th><th style="text-align:center;">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span style="padding:3px 10px;border-radius:20px;font-size:.72rem;font-weight:600;background:#eff6ff;color:#1d4ed8;">{{ $payout->account_type ?? '—' }}</span>
                        </td>
                        ...
                        <td style="text-align:center;">
                            <div style="display:flex;align-items:center;justify-content:center;gap:6px;">
                                <a href="{{ route('admin.payouts.edit',$payout) }}"
                                   style="padding:4px 10px;font-size:.72rem;font-weight:600;background:#dbeafe;color:#2563eb;border-radius:8px;text-decoration:none;"
                                   onmouseover="this.style.background='#bfdbfe'" onmouseout="this.style.background='#dbeafe'">Edit</a>
                                <form action="{{ route('admin.payouts.destroy',$payout) }}" method="POST" onsubmit="return confirm('Delete this payout?');" style="display:inline;">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                            style="padding:4px 10px;font-size:.72rem;font-weight:600;background:#fee2e2;color:#dc2626;border:none;border-radius:8px;cursor:pointer;"
                                            onmouseover="this.style.background='#fecaca'" onmouseout="this.style.background='#fee2e2'">Delete</button>
                                </form>
                            </div>
                        </td>
                    </tr>
        ...
        <div style="padding:1rem 1.5rem;border-top:1px solid #f1f5f9;">{{ $payouts->links() }}</div>
    </div>
