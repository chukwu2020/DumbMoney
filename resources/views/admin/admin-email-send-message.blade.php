...
<td style="background:#f0fdf4; border-left:4px solid #8bc905; padding:20px; border-radius:6px;">
  ...
  <!-- Original Message (only show if different) -->
  @if(isset($originalTitle) && $originalTitle != $subject)
  <details style="margin:20px 0; background:#f8f9fa; padding:15px; border-radius:8px;">
    <summary style="color:#0C3A30; cursor:pointer;">📝 View Original English Message</summary>
    <div style="margin-top:15px; padding-top:15px; border-top:1px dashed #8bc905;">
      <p style="margin:0 0 10px;"><strong>{{ $originalTitle }}</strong></p>
      <p style="margin:0; white-space: pre-line;">{{ $originalMessage ?? $messageBody }}</p>
    </div>
  </details>
  @endif
