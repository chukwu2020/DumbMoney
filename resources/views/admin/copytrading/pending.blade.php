...
<div class="dashboard-main-body">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div class="stats-card">
            ...
            <p class="text-2xl font-bold" style="color: #0C3A30;">{{ $pendingStats['total_pending'] ?? 0 }}</p>
        </div>
        <div class="stats-card" style="border-left-color: #10b981;">
            <p class="text-sm text-gray-500 mb-1">Total Approved</p>
            <p class="text-2xl font-bold text-green-600">{{ $approvedStats['total_approved'] ?? 0 }}</p>
        </div>
        <div class="stats-card" style="border-left-color: #ef4444;">
            <p class="text-sm text-gray-500 mb-1">Total Rejected</p>
            <p class="text-2xl font-bold text-red-600">{{ $rejectedStats['total_rejected'] ?? 0 }}</p>
        </div>
        <div class="stats-card" style="border-left-color: #8b5cf6;">
            <p class="text-sm text-gray-500 mb-1">Total Amount</p>
            <p class="text-2xl font-bold text-purple-600">
                ${{ number_format(($pendingStats['total_amount'] ?? 0) + ($approvedStats['total_amount'] ?? 0), 2) }}
            </p>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 gap-4">
          ...
          @include('admin.copytrading.partials', ['request' => $request, 'type' => 'pending'])
            ...
            <div class="empty-state">
                ...
                <h6 class="text-lg font-semibold text-gray-700 mb-2">No Pending Requests</h6>
                <p class="text-gray-500">All copy trading requests have been processed</p>
            </div>
            @endforelse
        </div>
    </div>
    ...
    <div id="approvedTab" class="tab-content hidden">
        <div class="grid grid-cols-1 gap-4">
            ...
            <div class="empty-state">
                ...
                <h6 class="text-lg font-semibold text-gray-700 mb-2">No Approved Requests</h6>
                <p class="text-gray-500">No copy trading requests have been approved yet</p>
            </div>
            @endforelse
        </div>
    </div>
    ...
    <div id="rejectedTab" class="tab-content hidden">
        <div class="grid grid-cols-1 gap-4">
            ...
            <div class="empty-state">
                <div class="empty-state-icon">
                    <iconify-icon icon="ph:x-circle-fill" class="text-4xl text-gray-400"></iconify-icon>
                </div>
                <h6 class="text-lg font-semibold text-gray-700 mb-2">No Rejected Requests</h6>
                <p class="text-gray-500">No copy trading requests have been rejected</p>
            </div>
            @endforelse
        </div>
    </div>
        ...
        <form id="rejectForm" method="POST">
            ...
            <div class="mb-4">
                ...
                <p class="text-xs text-gray-400 mt-1">
                    <iconify-icon icon="ph:info-fill" class="inline"></iconify-icon>
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" 
                        class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition flex items-center gap-2" style="color: red;">
...
function approveRequest(id) {
    if (confirm('Are you sure you want to approve this copy trading request?')) {
